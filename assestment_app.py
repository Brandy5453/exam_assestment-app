...
def grade_assignment(student_features, rubric_df):
    ...
    for _, row in rubric_df.iterrows():
        ...
        if max_points <= 0:
            results.append({'Criteria': criteria, 'Points Possible': 0, 'Points Earned': 0, 'Feedback': "No points allocated", 'Match %': "N/A"})
            continue
...
def display_results(results, total_earned, total_possible, student_info):
    ...
    st.dataframe(results_df.style.highlight_max(axis=0, subset=['Points Earned']).format({'Points Possible': '{:.1f}', 'Points Earned': '{:.1f}'}))
    st.subheader("Performance Breakdown")
    if not results_df.empty:
        viz_df = results_df.copy()
        viz_df['Match Numeric'] = viz_df['Match %'].apply(lambda x: float(x.replace('%','')) if x != 'N/A' else 0)
        col1, col2 = st.columns(2)
        with col1:
